...
export default function NewPost({ takeFocus, date, onSuccess, showAvatar }) {
  ...
  const shortMessage = message.substr(0, 30)
  useEffect(() => {
    document.title = shortMessage
  }, [shortMessage])

  // useEffect(f, []) // runs when no state changes
  // useEffect(f) // runs when all state changes
  // useEffect(f, [that, state]) // runs when that state changes
